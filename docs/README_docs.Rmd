...
```
Estimate linear impulse responses with package function *lp_lin*.  
```{r, message = FALSE }  
...
# Specifications for switching variable
  specs$switching      <- data_set_df$GDP_gap # The output gap 
...
The switching variable ($z$) can either be decomposed by the Hodrick-Prescott filter as proposed by [Auerbach and Gorodnichenko (2013)](http://www.nber.org/papers/w17447.pdf) or directly plugged into the following transition function:  
...
- Regime 1 ($R_1$): $Y_{t-i}\cdot(1-F(z_{t-1}))$, with $i = 1, ...p$,

- Regime 2 ($R_2$): $Y_{t-i}\cdot F(z_{t-1})$,   with $i = 1, ...p$.
...
First, a reduced VAR is estimated to obtain the covariance matrix of the residuals $\Sigma$. The Cholesky decomposition is then applied to obatin the shock matrix $D$. IRFs for both regimes are estimated via:

$$\hat{IR}^{R_1}(t,s,d_i) = \hat{B}_{1, R_1}^s d_i \ \ \ \ \ \ \ \ \ s = 0, ..., h, $$ 
and 

$$\hat{IR}^{R_2}(t,s,d_i) = \hat{B}_{1, R_2}^s d_i \ \ \ \ \ \ \ \ \ s = 0, ..., h, $$ 


with normalizations $\hat{B}_{1, R2}^0 = I$ and $\hat{B}_{1, R2}^0 = I$ and where $d_i$ is the $i^{th}$ column from $D$. $\hat{B}_{1, R1}^s$ and $\hat{B}_{1, R2}^s$ are obtained by running a sequence of OLS equation for equation forecasts (local projections):

$$ \bf y_{t+s} = \bf \alpha^s + B_{1, R_1}^s \left(Y_{t-1}\cdot(1-F(z_{t-1})\right) + ... +
    B_{1, R_1}^s \left(Y_{t-p}\cdot(1-F(z_{t-1})\right) + \\
B_{1, R_2}^s \left(Y_{t-1}\cdot F(z_{t-1}\right) + ... + B_{1, R_2}^s \left(Y_{t-p}\cdot F(z_{t-1}\right).
$$
