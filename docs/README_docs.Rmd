...
a data.frame(). 
```{r, message = FALSE }  
results_lin <- lp_lin(data_set_df, lags_lin       = 4L,  # Number of lags.
                                   ...
                                   confint        = 1.96, # Width of confidence bands: 1 = 68%, 1.67 = 90%, 
                                                          # 1.96 = 95%.
...
where $\gamma > 0$. To differentiate between the two regimes, the exogenous variables ($\boldsymbol{y}_{t-p}$) are multiplied with the values of the transition function above at *t − 1* where
...
results_nl <- lp_nl(data_set_df, lags_lin       = 4L,  # Number of lags for (reduced) linear VAR. 
                                 ...
                                 lambda         = 1600, # Monthly = 129600, Quarterly = 1600, Annual = 6.25.
...
# Make date sequence and store data in data_frame.
  ...
  data_df <- data.frame(x = dates, fz = fz, gdp_gap = specs$switching[(specs$lags_nl+1):length(data_set_df$FF)])
