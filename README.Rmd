...
economy model. These data are used by Jordà (2005) in chapter IV. 
...
- The first plot shows the response of the first variable (GDP_gap) to a shock in the first variable in. 
...
economy model. This data set is used by Jordà (2005) in chapter IV, p.174.
Please see the data's help file or the original paper for a detailed description. 
```{r, message = FALSE }
# Load data (from package)
  data_set_df <- interest_rules_var_data
...
This is done automatically in the function *create_nl_data*. 
...
Extract values of transition function from *results_nl* and make date sequence.
```{r, message = FALSE } 
  fz      <- results_nl$fz
...
- The first plot shows the response of the first variable (GDP_gap) to the shock in the first variable in regime 1. 
- The second plot shows the response of the first variable (GDP_gap) to the shock in the first in regime 2. 
```{r, fig.height = 3, fig.width = 4, fig.align = "center", message = FALSE }
# Load packages
  library(ggpubr)
  library(gridExtra)
