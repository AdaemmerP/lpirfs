...
economy model. This data set is used by Jordà (2005) in chapter IV. of his paper.
The data description can be found in the data's help file. 
```{r, message = FALSE }
...
economy model. This data set is used by Jordà (2005) in chapter IV.
The data description can be found in the data's help file. 
```{r, message = FALSE }
# Load data (from package)
  data_set_df <- interest_rules_var_data
...
To differentiate between two regimes, the defined switching variable (z) is plugged into the follwing
switching function:

- $F_{z_t} = \frac{exp(-\gamma z_t)}{1 + exp(-\gamma z_t)}$


WARNING: To avoid contemporaneous feedback from the switching variable, the index of z is set to *t − 1* (for details see Auerbach and Gorodnichenko; 2012). This is done automatically done in the function *create_nl_data*. If you do not want the switching function to be lagged, please provide the switching variable with a lead of one.

- Regime 1 is defined as: $X_{t-p}*(1-F(z_t))$
- Regime 2 is defined as: $X_{t-p}*(F(z_t))$
...
- The first plot shows the response of the first variable (GDP_gap) to the shock of the first variable in 'data_set_df'. 
- The second plot shows the response of the second variable (Inflation) to the shock of the first variable (GDP_gap) in 'data_set_df'. 
```{r, fig.height = 3, fig.width = 4, fig.align = "center", message = FALSE, imgcenter }
# Load packages
  library(ggpubr)
  library(gridExtra)
